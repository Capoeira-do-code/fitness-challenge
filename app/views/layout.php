    ...
    <?php if ($loggedIn && !$minimalAppShell): ?>
        <header class="topbar">
            ...
            <div class="topbar-actions">
                ...
                <a class="topbar-notif-btn topbar-notif-link" href="/?page=notifications"
                    aria-label="<?= e(t('nav.notifications')) ?><?= $unreadNotificationsCount > 0 ? ' (' . (int) $unreadNotificationsCount . ')' : '' ?>">
                ...
                <details class="notif-menu">
                    ...
                    <div class="notif-menu-panel" aria-labelledby="notif-preview-title">
                        <div class="notif-menu-head">
                            <span class="notif-menu-title"><strong
                                    id="notif-preview-title"><?= e(t('nav.notifications')) ?></strong><small><?= e(t('notifications.unread_count', ['count' => $unreadNotificationsCount])) ?></small></span>
                            <?php if ($unreadNotificationsCount > 0): ?>
                                <form method="post" action="/?page=notifications" class="inline-form">
                                    ...
                                    <button class="btn btn-ghost small"
                                        type="submit"><?= e(t('notifications.mark_all_read')) ?></button>
                                </form>
                            <?php endif; ?>
                        </div>
                        <?php if ($notifPreview === []): ?>
                                ...
                                <?php foreach ($notifPreview as $notifItem): ?>
                                    <li class="notif-menu-item<?= empty($notifItem['read_at']) ? ' is-unread' : '' ?>">
                                        <a href="/?page=notifications&open_notification_id=<?= (int) $notifItem['id'] ?>">
                                            ...
                                            <small
                                                class="muted"><?= e(human_time_ago((string) ($notifItem['created_at'] ?? ''))) ?></small>
                                        </a>
                                    </li>
                                <?php endforeach; ?>
                            </ul>
                        <?php endif; ?>
                        <a class="btn btn-ghost small btn-block notif-menu-all"
                            href="/?page=notifications"><?= e(t('common.view_all')) ?></a>
                    </div>
                </details>
                <details class="add-menu topbar-add-menu">
                    <summary class="btn btn-primary add-menu-trigger btn-add" data-add-button
                        aria-label="<?= e(t('entries.title')) ?>">
                ...
                <details class="user-menu">
                    <summary class="user-menu-trigger"
                        aria-label="<?= e(t('nav.user_menu', ['name' => (string) $currentUser['display_name']])) ?>"
                        aria-expanded="false">
                        ...
                        <?php if ($currentUserAvatarUrl !== ''): ?>
                            <img class="<?= e(trim($currentUserFrameClass)) ?>" src="<?= e($currentUserAvatarUrl) ?>"
                                alt="<?= e((string) $currentUser['display_name']) ?>">
                        <?php else: ?>
                            <span
                                class="<?= e(trim($currentUserFrameClass)) ?>"><?= e(initials_for((string) $currentUser['display_name'])) ?></span>
                        <?php endif; ?>
                    </summary>
                    <nav class="user-menu-panel"
                        aria-label="<?= e(t('nav.user_menu', ['name' => (string) $currentUser['display_name']])) ?>"
                        data-menu-stack>
                        <div class="user-menu-view<?= is_admin($currentUser) ? ' has-admin-item' : '' ?>"
                            data-menu-view="main">
                            <?php if (function_exists('xp_user_level_info')):
                                $menuXp = xp_user_level_info($GLOBALS['pdo'], (int) $currentUser['id']); ?>
                                <a class="user-menu-level" href="/?page=profile"
                                    title="<?= e(t('xp.level') . ' ' . (int) $menuXp['level']) ?>">
                                    <span class="profile-level-badge"><?= e(t('xp.level_short')) ?>
                                        <?= (int) $menuXp['level'] ?></span>
                                    <span class="user-menu-xp">
                                        <span class="user-menu-xp-bar"><span
                                                style="width: <?= max(0, min(100, (int) $menuXp['progress_pct'])) ?>%"></span></span>
                                        <span class="user-menu-xp-text"><?= e(number_format((int) $menuXp['total_xp'])) ?>
                                            <?= e(t('xp.points')) ?> &middot;
                                            <?= e(t('xp.to_next', ['xp' => number_format((int) $menuXp['xp_to_next'])])) ?></span>
                                    </span>
                                </a>
                            <?php endif; ?>
                            <a class="user-menu-link" href="/?page=profile" <?= $currentPage === 'profile' ? ' aria-current="page"' : '' ?>><span class="user-menu-item-icon"
                                    aria-hidden="true"><?= activity_icon_svg('user') ?></span><span><?= e(t('nav.profile')) ?></span></a>
                            <a class="user-menu-link" href="/?page=workouts" <?= $currentPage === 'workouts' ? ' aria-current="page"' : '' ?>><span class="user-menu-item-icon"
                                    aria-hidden="true"><?= activity_icon_svg('dumbbell') ?></span><span><?= e(t('nav.workouts')) ?></span></a>
                            <a class="user-menu-link" href="/?page=friends" <?= $currentPage === 'friends' ? ' aria-current="page"' : '' ?>><span class="user-menu-item-icon"
                                    aria-hidden="true"><?= activity_icon_svg('users') ?></span><span><?= e(t('nav.friends')) ?></span></a>
                            <a class="user-menu-link" href="/?page=settings" <?= $currentPage === 'settings' ? ' aria-current="page"' : '' ?>><span class="user-menu-item-icon"
                                    aria-hidden="true"><?= activity_icon_svg('sliders') ?></span><span><?= e(t('nav.settings')) ?></span></a>
                            <button type="button" class="user-menu-theme-toggle" data-theme-toggle
                                data-csrf="<?= e(csrf_token()) ?>" data-label-dark="<?= e(t('nav.theme_toggle_dark')) ?>"
                                data-label-light="<?= e(t('nav.theme_toggle_light')) ?>"
                                aria-pressed="<?= $themeMode === 'dark' ? 'true' : 'false' ?>">
                                <span class="theme-toggle-icon theme-toggle-icon-sun" aria-hidden="true"><svg
                                        viewBox="0 0 24 24">
                                        <circle cx="12" cy="12" r="4" />
                                        <path
                                            d="M12 2v2M12 20v2M4 12H2M22 12h-2M5 5l1.5 1.5M17.5 17.5 19 19M5 19l1.5-1.5M17.5 6.5 19 5" />
                                    </svg></span>
                                <span class="theme-toggle-icon theme-toggle-icon-moon" aria-hidden="true"><svg
                                        viewBox="0 0 24 24">
                                        <path d="M20 14.5A8 8 0 1 1 9.5 4a6.5 6.5 0 0 0 10.5 10.5z" />
                                    </svg></span>
                                <span class="theme-toggle-label"
                                    data-theme-toggle-label><?= e($themeMode === 'dark' ? t('nav.theme_toggle_light') : t('nav.theme_toggle_dark')) ?></span>
                            </button>
                            <?php if (is_admin($currentUser)): ?>
                                <a class="user-menu-link" href="/?page=admin" <?= $currentPage === 'admin' ? ' aria-current="page"' : '' ?>><span class="user-menu-item-icon"
                                        aria-hidden="true"><?= activity_icon_svg('shield') ?></span><span><?= e(t('nav.admin')) ?></span></a>
                            <?php endif; ?>
                            <a class="user-menu-link user-menu-logout" href="/?page=logout"><span
                                    class="user-menu-item-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none"
                                        stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                        stroke-linejoin="round">
                                        <path d="M10 17l5-5-5-5M15 12H3" />
                                        <path d="M14 4h5a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2h-5" />
                                    </svg></span><span><?= e(t('nav.logout')) ?></span></a>
                        </div>
                    </nav>
                </details>
            </div>
    ...
    <main class="container <?= $loggedIn && !$minimalAppShell ? 'container-with-nav' : '' ?>">
        ...
        <?php if ($loggedIn && !$minimalAppShell && $contextualBackFallback !== ''): ?>
            <nav class="contextual-route-back" data-contextual-back-container aria-label="<?= e(t('common.back')) ?>">
                <button class="hierarchy-back destination-back" type="button" data-hierarchy-back
                    data-fallback="<?= e($contextualBackFallback) ?>"
                    aria-label="<?= e(t('common.back')) ?>: <?= e($contextualBackDestination) ?>"><span
                        aria-hidden="true">&larr;</span><strong><?= e($contextualBackDestination) ?></strong></button>
            </nav>
        <?php endif; ?>
