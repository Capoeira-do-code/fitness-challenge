    ...
    <section class="admin-achievement-form-card">
        ...
        <div class="admin-achievement-rule-grid" data-achievement-conditional-fields <?= $achievementConditional ? '' : 'hidden' ?>>
            <label><span><?= e(t('achievements.metric')) ?></span><select name="metric" data-achievement-metric>
                <?php foreach (['steps' => t('metric.steps'), 'distance_km' => t('metric.distance_km'), 'workouts' => t('metric.workouts'), 'score' => t('metric.score'), 'strikes' => t('metric.strikes'), 'weight' => t('metric.weight'), 'strength_rank' => t('admin.achievements_metric_strength_rank'), 'habit_completion' => t('admin.habits')] as $metricValue => $metricLabel): ?><option value="<?= e($metricValue) ?>" <?= $achievementMetric === $metricValue ? 'selected' : '' ?>><?= e($metricLabel) ?></option><?php endforeach; ?>
                <?php if ($penaltiesEnabled || $achievementMetric === 'penalties'): ?><option value="penalties" <?= $achievementMetric === 'penalties' ? 'selected' : '' ?>><?= e(t('admin.penalties_feature')) ?></option><?php endif; ?>
            </select></label>
            <label data-achievement-habit-wrap <?= $achievementMetric === 'habit_completion' ? '' : 'hidden' ?>><span><?= e(t('admin.habits')) ?></span><select name="habit_code"><?php foreach (($habits ?? []) as $habit): ?><option value="<?= e((string) $habit['code']) ?>" <?= (string) $habit['code'] === $achievementHabitCode ? 'selected' : '' ?>><?= e((string) $habit['label']) ?></option><?php endforeach; ?></select></label>
