...
$workoutHubLabels = [
    'overview' => [t('workouts.title'), t('workouts.subtitle')],
...
$activeSessionPreviousSetCount = max(0, (int) ($activeSessionSummary['previous_sets_count'] ?? 0));
...
$activeElapsedSeconds = max(0, (int) ($activeSessionSummary['elapsed_seconds'] ?? ((int) ($activeSessionSummary['elapsed_minutes'] ?? 0) * 60)));
$activeElapsedMinutes = intdiv($activeElapsedSeconds, 60);
$activeElapsedLabel = $activeElapsedMinutes >= 60
    ...
    : ($activeElapsedMinutes > 0
        ...
        : t('workouts.elapsed_seconds', ['seconds' => $activeElapsedSeconds]));
...
$muscleSelectedList = array_values(array_filter(
    array_map('trim', explode(',', (string) ($filters['muscle'] ?? ''))),
    static fn(string $m): bool => $m !== ''
));
$libraryExercises = (array) ($wkLibrary ?? []);
$libraryExerciseMediaMap = (array) ($wkLibraryExerciseMedia ?? []);
...
$libraryTargetReturnUrl = $targetSessionId > 0
    ? '/?page=workouts&session_id=' . $targetSessionId
    : '/?page=workouts&routine_id=' . $targetRoutineId;
...
<section class="screen stack-lg workouts-screen workouts-view-<?= e($wkView) ?><?= $hasLibraryTarget ? ' has-library-target' : '' ?><?= $wkView === 'list' && $hasActiveWorkoutSession ? ' has-active-session-dock' : '' ?>">
    ...
    <?php if ($wkView === 'list'): ?>
        <nav class="workouts-training-commandbar" aria-label="<?= e(t('workouts.title')) ?>">
            <button type="button" data-app-modal-open="wk-new-routine-modal">
                <span aria-hidden="true"><?= activity_icon_svg('clipboard') ?></span>
                <strong><?= e(t('workouts.new_routine')) ?></strong>
            </button>
            <a href="/?page=workouts&amp;view=library">
                <span aria-hidden="true"><?= activity_icon_svg('search') ?></span>
                <strong><?= e(t('workouts.explore_exercises')) ?></strong>
            </a>
        </nav>
    <?php endif; ?>
            ...
            <?php if ($hasActiveWorkoutSession): ?>
                <article class="workouts-active-session-card compact-panel glass-panel compact-progress-panel" data-session-progress="<?= (int) ($activeSessionSummary['progress_pct'] ?? 0) ?>">
                    ...
                    <div class="workouts-active-session-progress">
                        ...
                        <span class="workouts-active-sets"><strong><?= (int) ($activeSessionSummary['completed_sets'] ?? 0) ?>/<?= (int) ($activeSessionSummary['total_sets'] ?? 0) ?></strong><small><?= e(t('workouts.sets')) ?></small></span>
                    ...
                    <div class="workouts-active-session-finish">
                        <button type="button" class="btn btn-ghost small" data-app-modal-open="wk-finish-session-modal" aria-haspopup="dialog"><span aria-hidden="true"><?= activity_icon_svg('check') ?></span><span><?= e(t('workouts.finish')) ?></span></button>
                    </div>
                </article>
            <?php endif; ?>

            <?php if ($todayRoutines !== [] && !$hasActiveWorkoutSession): ?>
                <article class="panel workouts-today-card compact-panel glass-panel">
                    ...
                    <form method="post" action="/?page=workouts" class="workouts-today-picker">
                        ...
                        <button class="btn btn-primary" type="submit"><?= e(t('workouts.start_routine')) ?></button>
                    </form>
                </article>
            <?php endif; ?>
        </div>
...
<?php elseif ($wkView === 'list'): ?>
    ...
    <?php if ($activeRoutines === []): ?>
        <section class="workouts-routines-empty-block" aria-labelledby="workouts-empty-routines-title">
            <header><span aria-hidden="true">⌄</span><h2 id="workouts-empty-routines-title"><?= e(t('workouts.your_routines')) ?></h2></header>
    ...
    <?php else: ?>
        <details class="workouts-overview-disclosure workouts-routines-disclosure" data-persist-disclosure="workouts.routines" open>
            <summary>
            ...
            <div class="workouts-routine-grid workouts-routine-preview-grid">
                <?php foreach ($activeRoutines as $routine): ?>
                    ...
                    $routineExerciseNames = array_values((array) ($routineExerciseNamePreviews[$rid] ?? []));
                    $routineExercisePreview = $routineExerciseNames !== []
                        ? implode(', ', $routineExerciseNames)
                        : trim((string) ($routine['description'] ?? ''));
                    if ($routineExercisePreview === '') {
                        $routineExercisePreview = t('workouts.add_exercises_hint');
                    }
                    ...
                    <article class="workouts-routine-card compact-list-item is-<?= e($routineStateKey) ?><?= (int) ($routine['is_favorite'] ?? 0) === 1 ? ' is-favorite' : '' ?><?= $routineCover !== null ? ' has-cover' : '' ?>" style="--routine-accent: <?= e($routineAccent) ?>" data-state="<?= e($routineStateKey) ?>">
                        ...
                        <a class="workouts-routine-preview-copy" href="/?page=workouts&amp;routine_id=<?= $rid ?>">
                            ...
                            <p class="workouts-routine-exercise-preview"><?= e($routineExercisePreview) ?></p>
            ...
            <button type="button" class="workouts-routine-add-card" data-app-modal-open="wk-new-routine-modal"><span aria-hidden="true">+</span><strong><?= e(t('workouts.create_new_routine')) ?></strong></button>
            </div>
        </details>
    <?php endif; ?>
...
<?php elseif ($wkView === 'library'): ?>
    <?php if ($hasLibraryTarget): ?>
        <aside class="workouts-library-target" aria-label="<?= e(t('workouts.add_exercises')) ?>">
            <a class="workouts-picker-cancel" href="<?= e($libraryTargetReturnUrl) ?>"><?= e(t('common.cancel')) ?></a>
            ...
            <div><small><?= e($targetSessionId > 0 ? t('workouts.adding_to_session') : t('workouts.add_exercises_to')) ?></small><strong class="workouts-library-target-name"><?= e($targetSessionId > 0 ? ((string) ($targetSession['title'] ?? '') !== '' ? (string) $targetSession['title'] : t('workouts.active_session')) : (string) ($targetRoutine['name'] ?? '')) ?></strong><strong class="workouts-picker-title"><?= e(t('workouts.add_exercise')) ?></strong></div>
            <a class="btn btn-primary small workouts-picker-create workouts-picker-save" href="<?= e($libraryTargetReturnUrl) ?>"><?= e(t('common.save')) ?></a>
        </aside>
    <?php endif; ?>
    ...
    <article class="panel workouts-library-head">
        ...
        <form method="get" action="/" id="workouts-library-search" class="workouts-library-mobile-search workouts-library-quick-search<?= ($hasLibrarySearch || $hasLibraryTarget) ? ' is-open' : '' ?>" data-workout-search-panel aria-hidden="<?= ($hasLibrarySearch || $hasLibraryTarget) ? 'false' : 'true' ?>">
            ...
            <label class="workouts-library-search-field"><span class="sr-only"><?= e(t('workouts.search_exercises')) ?></span><input type="search" name="q" value="<?= e((string) ($filters['q'] ?? '')) ?>" placeholder="<?= e(t('workouts.search_exercises')) ?>"><?php if ($hasAnyLibraryFilter): ?><a class="workouts-library-search-clear" href="<?= e($libraryClearUrl) ?>" aria-label="<?= e(t('common.clear')) ?>"><span aria-hidden="true">&times;</span></a><?php endif; ?></label>
        ...
        <?php if ($hasLibraryTarget): ?>
            ...
            <button class="workouts-picker-custom-create" type="button" data-app-modal-open="wk-create-exercise-leave-modal" aria-haspopup="dialog">
                ...
                <span><strong><?= e(t('workouts.create_custom')) ?></strong><small><?= e(t('workouts.create_custom_picker_hint')) ?></small></span>
                <b aria-hidden="true">&rsaquo;</b>
            </button>
        <?php endif; ?>
        <div class="workouts-library-results<?= $hasAnyLibraryFilter ? ' is-filtered' : '' ?>" aria-live="polite">
            ...
            <div>
                ...
                <?php if ($hasAnyLibraryFilter): ?>
                    <small><?php if ($hasLibrarySearch): ?><span>&ldquo;<?= e(trim((string) $filters['q'])) ?>&rdquo;</span><?php endif; ?><?php if ($muscleSelectedList !== []): ?><span><?= e(implode(', ', array_map($muscleLabel, $muscleSelectedList))) ?></span><?php endif; ?><?php if ((string) ($filters['equipment'] ?? '') !== ''): ?><span><?= e($equipmentLabel((string) $filters['equipment'])) ?></span><?php endif; ?><?php if ((string) ($filters['context'] ?? '') !== ''): ?><span><?= e($contextLabel((string) $filters['context'])) ?></span><?php endif; ?></small>
                <?php else: ?>
                    <small><?= e(t('workouts.library_subtitle')) ?></small>
                <?php endif; ?>
            </div>
            <?php if ($hasAnyLibraryFilter): ?><a href="<?= e($libraryClearUrl) ?>"><?= e(t('common.clear')) ?></a><?php endif; ?>
        </div>
    </article>
    <div class="workouts-filter-sheet" id="workouts-library-filters" data-workout-filter-panel aria-hidden="true">
        ...
        <form method="get" action="/" class="workouts-library-filters">
            ...
            <div class="workouts-muscle-filters" aria-label="<?= e(t('workouts.all_body_parts')) ?>" data-workout-muscle-filters>
                <label class="workouts-muscle-filter-all"><input type="checkbox" data-workout-muscle-all<?= $muscleSelectedList === [] ? ' checked' : '' ?>><span><?= e(t('workouts.all_body_parts')) ?></span></label>
                <?php foreach ((array) ($wkMuscleGroups ?? []) as $muscle): ?>
                    <label><input type="checkbox" name="muscle[]" value="<?= e((string) $muscle) ?>"<?= in_array((string) $muscle, $muscleSelectedList, true) ? ' checked' : '' ?>><span><?= e($muscleLabel((string) $muscle)) ?></span></label>
                <?php endforeach; ?>
            </div>
    ...
    <?php else: ?>
        ...
        ?>
        <?php if ($hasLibraryTarget): ?><h2 class="workouts-picker-section-title"><?= e(t('workouts.all_exercises')) ?></h2><?php endif; ?>
        <div class="workouts-library-grid<?= $hasLibraryTarget ? ' is-contextual' : '' ?><?= $useCompactLibrary ? ' is-compact' : '' ?>"
            ...
            <?php foreach ($libraryExercises as $exercise): ?>
                ...
                $libraryExerciseId = (int) ($exercise['id'] ?? 0);
                $libraryExerciseName = (string) ($exercise['display_name'] ?? $exercise['name'] ?? '');
                $libraryExerciseDetailId = 'workout-library-exercise-detail-' . $libraryExerciseId;
                $libraryExerciseGallery = array_values((array) ($libraryExerciseMediaMap[$libraryExerciseId] ?? []));
                $librarySecondaryDecoded = json_decode((string) ($exercise['secondary_muscles'] ?? '[]'), true);
                $librarySecondaryMuscles = array_values(array_filter(array_map('strval', is_array($librarySecondaryDecoded) ? $librarySecondaryDecoded : [])));
                ?>
                <article class="workouts-library-card<?= $isPersonalExercise ? ' is-personal' : '' ?><?= $isFavoriteExercise ? ' is-favorite' : '' ?>" style="<?= e($workoutExerciseStyle((array) $exercise)) ?>" data-exercise-id="<?= $libraryExerciseId ?>" data-muscle="<?= e((string) ($exercise['muscle_group'] ?? '')) ?>" data-cover-mode="<?= e($exerciseCoverMode) ?>" data-workout-exercise-detail-open="<?= e($libraryExerciseDetailId) ?>">
                    <?php $libraryImageUrl = $useExercisePhoto ? media_thumbnail_url($exerciseImagePath, 400) : ''; ?>
                    <?php if ($libraryImageUrl !== ''): ?>
                        <a class="workouts-library-media" href="<?= e($exerciseGuideUrl) ?>" aria-label="<?= e($libraryExerciseName) ?>" data-workout-exercise-detail-open="<?= e($libraryExerciseDetailId) ?>" aria-haspopup="dialog" aria-controls="<?= e($libraryExerciseDetailId) ?>">
                    ...
                    <?php elseif ($useVideoThumbnail): ?>
                        <a class="workouts-library-media is-video-thumbnail" href="<?= e($exerciseGuideUrl) ?>" aria-label="<?= e($libraryExerciseName) ?>" style="--workout-video-thumb: url('<?= e($libraryVideoThumbnail) ?>')" data-workout-exercise-detail-open="<?= e($libraryExerciseDetailId) ?>" aria-haspopup="dialog" aria-controls="<?= e($libraryExerciseDetailId) ?>">
                    ...
                    <div class="workouts-library-copy">
                        <h3><a class="workouts-library-name-link" href="<?= e($exerciseGuideUrl) ?>" data-workout-exercise-detail-open="<?= e($libraryExerciseDetailId) ?>" aria-haspopup="dialog" aria-controls="<?= e($libraryExerciseDetailId) ?>"><?= e($libraryExerciseName) ?></a></h3>
                    ...
                    <div class="workouts-library-card-foot">
                        <div class="inline-actions"><a class="btn btn-ghost small" href="<?= e($exerciseGuideUrl) ?>" data-workout-exercise-detail-open="<?= e($libraryExerciseDetailId) ?>" aria-haspopup="dialog" aria-controls="<?= e($libraryExerciseDetailId) ?>"><?= e(t('workouts.view_guide')) ?></a><?php if ($isPersonalExercise): ?><a class="btn btn-ghost small" href="<?= e($libraryUrl(['custom_exercise' => (int) $exercise['id'], 'library_page' => null])) ?>"><?= e(t('common.edit')) ?></a><?php endif; ?></div>
                        <?php if ($targetSessionId > 0): ?>
                            <form method="post" action="/?page=workouts" class="workouts-library-add is-contextual" data-workout-ajax-add>
                                ...
                                <button class="btn <?= $isInLibraryTarget ? 'btn-ghost is-added' : 'btn-primary' ?> small" type="submit"<?= $isInLibraryTarget ? ' disabled' : '' ?> data-added-label="<?= e(t('workouts.added_to_session')) ?>"><?= e($isInLibraryTarget ? t('workouts.added_to_session') : t('workouts.add_to_session')) ?></button>
                            </form>
                        <?php elseif ($targetRoutineId > 0): ?>
                            <form method="post" action="/?page=workouts" class="workouts-library-add is-contextual" data-workout-ajax-add>
                                ...
                                <input type="hidden" name="muscle" value="<?= e((string) ($filters['muscle'] ?? '')) ?>"><input type="hidden" name="equipment" value="<?= e((string) ($filters['equipment'] ?? '')) ?>"><input type="hidden" name="context" value="<?= e((string) ($filters['context'] ?? '')) ?>"><input type="hidden" name="q" value="<?= e((string) ($filters['q'] ?? '')) ?>"><input type="hidden" name="scope" value="<?= e((string) ($filters['scope'] ?? '')) ?>"><input type="hidden" name="library_page" value="<?= (int) ($wkLibraryPage ?? 1) ?>">
                                <button class="btn <?= $isInLibraryTarget ? 'btn-ghost is-added' : 'btn-primary' ?> small" type="submit"<?= $isInLibraryTarget ? ' disabled' : '' ?> data-added-label="<?= e(t('workouts.added_to_routine')) ?>"><?= e($isInLibraryTarget ? t('workouts.added_to_routine') : t('workouts.add_to_routine')) ?></button>
                            </form>
                    ...
                    <div class="app-modal workouts-exercise-detail-modal workouts-library-exercise-detail-modal" id="<?= e($libraryExerciseDetailId) ?>" hidden role="dialog" aria-modal="true" aria-labelledby="<?= e($libraryExerciseDetailId) ?>-title">
                        <div class="app-modal-card workouts-exercise-detail-sheet workouts-library-exercise-detail-sheet">
                            <div class="workouts-exercise-detail-grabber" aria-hidden="true"></div>
                            <div class="workouts-exercise-detail-head"><h2 id="<?= e($libraryExerciseDetailId) ?>-title"><?= e($libraryExerciseName) ?></h2><button type="button" data-app-modal-close aria-label="<?= e(t('common.close_action')) ?>">&times;</button></div>
                            <?php if ($libraryExerciseGallery !== []): ?>
                                <div class="workouts-exercise-detail-gallery">
                                    <?php foreach ($libraryExerciseGallery as $galleryIndex => $galleryItem): ?><figure<?= $galleryIndex === 0 ? ' class="is-primary"' : '' ?>><img src="<?= e(media_thumbnail_url((string) ($galleryItem['path'] ?? ''), $galleryIndex === 0 ? 900 : 320)) ?>" alt="<?= e((string) ($galleryItem['caption'] ?? '')) ?>" loading="lazy" style="object-position: <?= e(wk_image_position_css($galleryItem['position'] ?? 'center')) ?>"><?php if (trim((string) ($galleryItem['caption'] ?? '')) !== ''): ?><figcaption><?= e((string) $galleryItem['caption']) ?></figcaption><?php endif; ?></figure><?php endforeach; ?>
                                </div>
                            <?php elseif ($exerciseImagePath !== ''): ?><img class="workouts-exercise-detail-cover" src="<?= e(media_thumbnail_url($exerciseImagePath, 900)) ?>" alt="<?= e($libraryExerciseName) ?>" loading="lazy" style="object-position: <?= e($exerciseImagePosition) ?>"><?php endif; ?>
                            <?php if (trim((string) ($content['summary'] ?? '')) !== ''): ?><p class="workouts-exercise-detail-summary"><?= e((string) $content['summary']) ?></p><?php endif; ?>
                            <dl class="workouts-exercise-detail-meta">
                                <div><dt><?= e(t('workouts.primary_muscle')) ?></dt><dd><?= e($muscleLabel((string) ($exercise['muscle_group'] ?? ''))) ?></dd></div>
                                <?php if ($librarySecondaryMuscles !== []): ?><div><dt><?= e(t('workouts.secondary_muscles')) ?></dt><dd><?= e(implode(', ', array_map($muscleLabel, $librarySecondaryMuscles))) ?></dd></div><?php endif; ?>
                                <div><dt><?= e(t('workouts.equipment')) ?></dt><dd><?= e($equipmentLabel((string) ($exercise['equipment'] ?? ''))) ?></dd></div>
                                <div><dt><?= e(t('workouts.exercise_type')) ?></dt><dd><?= e($exerciseTypeLabel((string) ($exercise['exercise_type'] ?? 'strength'))) ?></dd></div>
                                <div><dt><?= e(t('workouts.difficulty')) ?></dt><dd><?= e($difficultyLabel((string) ($exercise['difficulty'] ?? 'beginner'))) ?></dd></div>
                            </dl>
                            <div class="workouts-exercise-detail-actions<?= $hasLibraryTarget ? '' : ' is-single' ?>">
                                <a class="btn btn-ghost" href="/?page=workouts&amp;view=stats&amp;exercise_stats=<?= $libraryExerciseId ?>"><?= activity_icon_svg('chart') ?><span><?= e(t('workouts.see_your_stats')) ?></span></a>
                                <?php if ($hasLibraryTarget): ?><button class="btn <?= $isInLibraryTarget ? 'btn-ghost is-added' : 'btn-primary' ?>" type="button" data-workout-library-detail-add<?= $isInLibraryTarget ? ' disabled' : '' ?>><span aria-hidden="true"><?= activity_icon_svg($isInLibraryTarget ? 'check' : 'plus') ?></span><span data-workout-library-detail-add-label><?= e($isInLibraryTarget ? ($targetSessionId > 0 ? t('workouts.added_to_session') : t('workouts.added_to_routine')) : ($targetSessionId > 0 ? t('workouts.add_to_session') : t('workouts.add_to_routine'))) ?></span></button><?php endif; ?>
                            </div>
                            <?php if ($exerciseVideoSource !== null): ?><div class="workouts-exercise-detail-video"><?php if (($exerciseVideoSource['type'] ?? '') === 'iframe'): ?><iframe src="<?= e((string) ($exerciseVideoSource['url'] ?? '')) ?>" title="<?= e($libraryExerciseName) ?>" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe><?php elseif (($exerciseVideoSource['type'] ?? '') === 'video'): ?><video src="<?= e((string) ($exerciseVideoSource['url'] ?? '')) ?>" controls preload="metadata"></video><?php else: ?><a class="btn btn-ghost" href="<?= e((string) ($exerciseVideoSource['url'] ?? '')) ?>" target="_blank" rel="noopener noreferrer"><?= e(t('workouts.watch_video')) ?> &nearr;</a><?php endif; ?></div><?php endif; ?>
                            <?php if ((array) ($content['steps'] ?? []) !== []): ?><section class="workouts-exercise-detail-how"><h3><?= e(t('workouts.how_to')) ?></h3><ol><?php foreach ((array) $content['steps'] as $step): ?><li><?= e((string) $step) ?></li><?php endforeach; ?></ol></section><?php endif; ?>
                            <?php if ((array) ($content['tips'] ?? []) !== []): ?><section class="workouts-exercise-detail-notes"><h3><?= e(t('workouts.tips')) ?></h3><ul><?php foreach ((array) $content['tips'] as $tip): ?><li><?= e((string) $tip) ?></li><?php endforeach; ?></ul></section><?php endif; ?>
                            <button class="btn btn-primary workouts-exercise-detail-done" type="button" data-app-modal-close><?= e(t('workouts.done')) ?></button>
                        </div>
                    </div>
                </article>
            <?php endforeach; ?>
...
<?php elseif ($wkView === 'session' && !empty($wkSession)): ?>
    ...
    $activeSessionExerciseHasCompletedSets = count(array_filter(
        (array) ($activeSessionExercise['sets'] ?? []),
        static fn(array $set): bool => (int) ($set['completed'] ?? 0) === 1
    )) > 0;
    $sessionExerciseDefinitionIds = array_fill_keys(array_map(
        static fn(array $exercise): int => (int) ($exercise['exercise_def_id'] ?? 0),
        $sessionExercises
    ), true);
    $sessionReplacementCandidates = array_values(array_filter(
        $exercises,
        static fn(array $exercise): bool => !isset($sessionExerciseDefinitionIds[(int) ($exercise['id'] ?? 0)])
    ));
    usort($sessionReplacementCandidates, static function (array $left, array $right) use ($activeSessionExercise): int {
        $score = static function (array $candidate) use ($activeSessionExercise): int {
            return ((string) ($candidate['muscle_group'] ?? '') === (string) ($activeSessionExercise['muscle_group'] ?? '') ? 2 : 0)
                + ((string) ($candidate['equipment'] ?? '') === (string) ($activeSessionExercise['equipment'] ?? '') ? 1 : 0);
        };
    ...
    <article class="workouts-session-panel<?= $sessionRoutineCover !== null ? ' has-routine-cover' : '' ?>" style="--routine-accent: <?= e($sessionRoutineAccent) ?>">
        <div class="panel-head workouts-session-panel-head">
            <a class="workouts-session-panel-back" href="/?page=workouts" aria-label="<?= e(t('common.back')) ?>: <?= e(t('workouts.title')) ?>"><span aria-hidden="true"><?= activity_icon_svg('chevron-left') ?></span><strong><?= e(t('workouts.title')) ?></strong></a>
            <div class="workouts-session-title"><?php if ($sessionRoutineCover !== null): ?><span class="workouts-session-routine-cover" aria-hidden="true"><img src="<?= e((string) $sessionRoutineCover['url']) ?>" alt="" style="object-position: <?= e($workoutCoverPosition($sessionRoutineCover)) ?>"><?php if (!empty($sessionRoutineCover['has_video'])): ?><b>&#9654;</b><?php endif; ?></span><?php endif; ?><div><p class="eyebrow"><?= e(t('workouts.active_session')) ?></p><h2><?= e(wk_session_display_title($s) !== '' ? wk_session_display_title($s) : t('workouts.session')) ?></h2></div></div>
            <?php if (!$sessionDone): ?>
                <div class="workouts-session-head-actions">
                    ...
                    <details class="kebab-menu workouts-session-more" data-kebab-menu data-align="end">
                        ...
                        <div class="kebab-menu-panel" role="menu">
                            <button type="button" class="kebab-menu-item" data-app-modal-open="wk-finish-session-modal" aria-haspopup="dialog"><?= e(t('workouts.finish')) ?></button>
                            <?php if (!$activeSessionExerciseHasCompletedSets && $sessionReplacementCandidates !== []): ?>
                                <button type="button" class="kebab-menu-item" data-app-modal-open="wk-replace-exercise-modal" data-workout-replace-open data-session-exercise-id="<?= (int) ($activeSessionExercise['id'] ?? 0) ?>" data-current-muscle="<?= e((string) ($activeSessionExercise['muscle_group'] ?? '')) ?>" data-current-equipment="<?= e((string) ($activeSessionExercise['equipment'] ?? '')) ?>" aria-haspopup="dialog"><?= e(t('workouts.replace_exercise')) ?></button>
                            <?php elseif ($activeSessionExerciseHasCompletedSets): ?>
                                <button type="button" class="kebab-menu-item" disabled aria-disabled="true" title="<?= e(t('workouts.replace_completed_locked')) ?>"><?= e(t('workouts.replace_exercise')) ?></button>
                            <?php endif; ?>
        ...
        <?php else: ?>
            <?php if (count($sessionExercises) > 1): ?>
                ...
                <nav class="workouts-session-exercise-nav" style="<?= e($workoutExerciseStyle($activeSessionExercise)) ?>" aria-label="<?= e(t('workouts.session_exercise_navigation')) ?>">
                    <div class="workouts-session-nav-progress" aria-label="<?= e(t('workouts.session_progress', ['completed' => $sessionCompletedSetCount, 'total' => $sessionTotalSetCount])) ?>">
                        <strong><?= $sessionCompletedSetCount ?> / <?= $sessionTotalSetCount ?> <?= e(t('workouts.sets')) ?></strong>
                        <span class="goal-progress"><span style="width: <?= $sessionSetProgress ?>%"></span></span>
                        <strong><?= $sessionSetProgress ?>%</strong>
                    </div>
            ...
            <?php if (!$sessionDone && $sessionHasRestTargets): ?>
                ...
                $workoutRestTimerExerciseId = (int) ($activeSessionExercise['id'] ?? 0);
            ...
            <div class="workouts-session-exercises">
                <?php foreach ($sessionExercises as $sessionExerciseIndex => $ex): ?>
                    ...
                    $sessionExerciseDetailId = 'workout-session-exercise-detail-' . (int) ($ex['id'] ?? 0);
                    $sessionExerciseGuide = (array) ($ex['content'] ?? []);
                    $sessionSecondaryDecoded = json_decode((string) ($ex['secondary_muscles'] ?? '[]'), true);
                    $sessionSecondaryMuscles = array_values(array_filter(array_map('strval', is_array($sessionSecondaryDecoded) ? $sessionSecondaryDecoded : [])));
                    $sessionExerciseNotes = trim((string) ($ex['notes'] ?? ''));
                    ...
                    <article class="workouts-session-exercise<?= (int) ($ex['id'] ?? 0) === $activeSessionExerciseId ? ' is-active' : '' ?>" style="<?= e($workoutExerciseStyle((array) $ex)) ?>" data-exercise-type="<?= e($sessionExerciseType) ?>" data-session-exercise-id="<?= (int) ($ex['id'] ?? 0) ?>">
                        <button class="workouts-session-exercise-head" type="button" data-app-modal-open="<?= e($sessionExerciseDetailId) ?>" data-workout-exercise-detail-open="<?= e($sessionExerciseDetailId) ?>" aria-haspopup="dialog" aria-controls="<?= e($sessionExerciseDetailId) ?>" aria-label="<?= e((string) $ex['exercise_name']) ?>">
                            ...
                            <span class="workouts-session-exercise-kind"><small><?= e($exerciseTypeLabel($sessionExerciseType)) ?></small><b aria-hidden="true"><?= activity_icon_svg('chevron-right') ?></b></span>
                        </button>
                            ...
                            <?php foreach ($sessionSetRows as $set): ?>
                                ...
                                <form method="post" action="/?page=workouts" class="workouts-set-row<?= (int) ($set['completed'] ?? 0) === 1 ? ' is-done' : '' ?><?= $sessionCanDeleteSet ? ' can-delete' : '' ?>" data-workout-set-form data-session-id="<?= $sid ?>" data-rest-seconds="<?= max(0, (int) ($ex['rest_seconds'] ?? 0)) ?>">
                                    ...
                                    <?php if ($sessionIsCardio): ?>
                                        ...
                                        <label class="workouts-set-field"><span class="sr-only"><?= e(t('workouts.duration_minutes')) ?></span><input type="text" name="duration_minutes" value="<?= $durationValue !== null ? e(rtrim(rtrim(number_format((int) $durationValue / 60, 1, '.', ''), '0'), '.')) : '' ?>" placeholder="<?= e(t('workouts.minutes_placeholder')) ?>" inputmode="decimal" pattern="[0-9]+([.,][0-9]+)?" autocomplete="off" data-workout-decimal<?= $sessionSetSuggested('duration') ? ' data-clear-workout-suggestion="1"' : '' ?>></label>
                                        <span class="workouts-set-x">·</span>
                                        <?php $distanceValue = $sessionSetValue('distance'); ?>
                                        <label class="workouts-set-field"><span class="sr-only"><?= e(t('workouts.distance_km')) ?></span><input type="text" name="distance" value="<?= $distanceValue !== null ? e(rtrim(rtrim(number_format((float) $distanceValue, 2, '.', ''), '0'), '.')) : '' ?>" placeholder="<?= e(t('workouts.km_placeholder')) ?>" inputmode="decimal" pattern="[0-9]+([.,][0-9]+)?" autocomplete="off" data-workout-decimal<?= $sessionSetSuggested('distance') ? ' data-clear-workout-suggestion="1"' : '' ?>></label>
                                    <?php elseif ($sessionIsIsometric): ?>
                                        ...
                                        <label class="workouts-set-field"><span class="sr-only"><?= e(t('workouts.weight')) ?></span><input type="text" name="weight" value="<?= $weightValue !== null ? e(rtrim(rtrim(number_format((float) $weightValue, 1, '.', ''), '0'), '.')) : '' ?>" placeholder="<?= e(t('workouts.weight')) ?>" inputmode="decimal" pattern="[0-9]+([.,][0-9]+)?" autocomplete="off" data-workout-decimal<?= $sessionSetSuggested('weight') ? ' data-clear-workout-suggestion="1"' : '' ?>></label>
                                    <?php else: ?>
                                        <?php $weightValue = $sessionSetValue('weight'); ?>
                                        <label class="workouts-set-field"><span class="sr-only"><?= e(t('workouts.weight')) ?></span><input type="text" name="weight" value="<?= $weightValue !== null ? e(rtrim(rtrim(number_format((float) $weightValue, 1, '.', ''), '0'), '.')) : '' ?>" placeholder="<?= e(t('workouts.weight')) ?>" inputmode="decimal" pattern="[0-9]+([.,][0-9]+)?" autocomplete="off" data-workout-decimal<?= $sessionSetSuggested('weight') ? ' data-clear-workout-suggestion="1"' : '' ?>></label>
                                        <span class="workouts-set-x">×</span>
                                    ...
                                    <?php endif; ?>
                                    <button type="submit" name="completed" value="<?= (int) ($set['completed'] ?? 0) === 1 ? '0' : '1' ?>" class="btn workouts-set-done<?= (int) ($set['completed'] ?? 0) === 1 ? ' is-done btn-primary' : ' btn-ghost' ?> small" aria-label="<?= e(t('workouts.done')) ?>" data-workout-set-toggle data-next-completed="<?= (int) ($set['completed'] ?? 0) === 1 ? '0' : '1' ?>"><?= activity_icon_svg('check') ?></button>
                        ...
                        <?php if ($sessionDone && $followingExercise !== null): ?><a class="workouts-session-next-exercise" href="<?= e($sessionExerciseUrl((int) $followingExercise['id'])) ?>"><span><small><?= e(t('workouts.next_exercise')) ?></small><strong><?= e((string) ($followingExercise['exercise_name'] ?? '')) ?></strong></span><b aria-hidden="true">&rarr;</b></a><?php endif; ?>
                    </article>
                    <div class="app-modal workouts-exercise-detail-modal" id="<?= e($sessionExerciseDetailId) ?>" hidden role="dialog" aria-modal="true" aria-labelledby="<?= e($sessionExerciseDetailId) ?>-title">
                        <div class="app-modal-card workouts-exercise-detail-sheet">
                            <div class="workouts-exercise-detail-grabber" aria-hidden="true"></div>
                            <div class="workouts-exercise-detail-head"><h2 id="<?= e($sessionExerciseDetailId) ?>-title"><?= e((string) ($ex['exercise_name'] ?? '')) ?></h2><button type="button" data-app-modal-close aria-label="<?= e(t('common.close_action')) ?>">&times;</button></div>
                            <?php if ($sessionExerciseGallery !== []): ?>
                                <div class="workouts-exercise-detail-gallery">
                                    <?php foreach ($sessionExerciseGallery as $galleryIndex => $galleryItem): ?><figure<?= $galleryIndex === 0 ? ' class="is-primary"' : '' ?>><img src="<?= e(media_thumbnail_url((string) ($galleryItem['path'] ?? ''), $galleryIndex === 0 ? 900 : 320)) ?>" alt="<?= e((string) ($galleryItem['caption'] ?? '')) ?>" loading="lazy" style="object-position: <?= e(wk_image_position_css($galleryItem['position'] ?? 'center')) ?>"><?php if (trim((string) ($galleryItem['caption'] ?? '')) !== ''): ?><figcaption><?= e((string) $galleryItem['caption']) ?></figcaption><?php endif; ?></figure><?php endforeach; ?>
                                </div>
                            <?php elseif ($sessionExerciseImagePath !== ''): ?><img class="workouts-exercise-detail-cover" src="<?= e(media_thumbnail_url($sessionExerciseImagePath, 900)) ?>" alt="<?= e((string) ($ex['exercise_name'] ?? '')) ?>" loading="lazy" style="object-position: <?= e(wk_image_position_css($ex['image_position'] ?? 'center')) ?>"><?php endif; ?>
                            <?php if (trim((string) ($sessionExerciseGuide['summary'] ?? '')) !== ''): ?><p class="workouts-exercise-detail-summary"><?= e((string) $sessionExerciseGuide['summary']) ?></p><?php endif; ?>
                            <dl class="workouts-exercise-detail-meta">
                                <div><dt><?= e(t('workouts.primary_muscle')) ?></dt><dd><?= e($muscleLabel((string) ($ex['muscle_group'] ?? ''))) ?></dd></div>
                                <?php if ($sessionSecondaryMuscles !== []): ?><div><dt><?= e(t('workouts.secondary_muscles')) ?></dt><dd><?= e(implode(', ', array_map($muscleLabel, $sessionSecondaryMuscles))) ?></dd></div><?php endif; ?>
                                <div><dt><?= e(t('workouts.equipment')) ?></dt><dd><?= e($equipmentLabel((string) ($ex['equipment'] ?? ''))) ?></dd></div>
                                <div><dt><?= e(t('workouts.exercise_type')) ?></dt><dd><?= e($exerciseTypeLabel($sessionExerciseType)) ?></dd></div>
                                <div><dt><?= e(t('workouts.difficulty')) ?></dt><dd><?= e($difficultyLabel((string) ($ex['difficulty'] ?? 'beginner'))) ?></dd></div>
                                <?php if ((int) ($ex['rest_seconds'] ?? 0) > 0): ?><div><dt><?= e(t('workouts.rest_time')) ?></dt><dd><?= e(t('workouts.rest_short', ['count' => (int) $ex['rest_seconds']])) ?></dd></div><?php endif; ?>
                            </dl>
                            <div class="workouts-exercise-detail-actions is-single"><a class="btn btn-ghost" href="/?page=workouts&amp;view=stats&amp;exercise_stats=<?= (int) ($ex['exercise_def_id'] ?? 0) ?>"><?= activity_icon_svg('chart') ?><span><?= e(t('workouts.see_your_stats')) ?></span></a></div>
                            <?php if ($sessionExerciseVideo !== null): ?><div class="workouts-exercise-detail-video"><?php if (($sessionExerciseVideo['type'] ?? '') === 'iframe'): ?><iframe src="<?= e((string) ($sessionExerciseVideo['url'] ?? '')) ?>" title="<?= e((string) ($ex['exercise_name'] ?? '')) ?>" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe><?php elseif (($sessionExerciseVideo['type'] ?? '') === 'video'): ?><video src="<?= e((string) ($sessionExerciseVideo['url'] ?? '')) ?>" controls preload="metadata"></video><?php else: ?><a class="btn btn-ghost" href="<?= e((string) ($sessionExerciseVideo['url'] ?? '')) ?>" target="_blank" rel="noopener noreferrer"><?= e(t('workouts.watch_video')) ?> &nearr;</a><?php endif; ?></div><?php endif; ?>
                            <?php if ((array) ($sessionExerciseGuide['steps'] ?? []) !== []): ?><section class="workouts-exercise-detail-how"><h3><?= e(t('workouts.how_to')) ?></h3><ol><?php foreach ((array) $sessionExerciseGuide['steps'] as $step): ?><li><?= e((string) $step) ?></li><?php endforeach; ?></ol></section><?php endif; ?>
                            <?php if ((array) ($sessionExerciseGuide['tips'] ?? []) !== []): ?><section class="workouts-exercise-detail-notes"><h3><?= e(t('workouts.tips')) ?></h3><ul><?php foreach ((array) $sessionExerciseGuide['tips'] as $tip): ?><li><?= e((string) $tip) ?></li><?php endforeach; ?></ul></section><?php endif; ?>
                            <?php if ($sessionExerciseNotes !== ''): ?><section class="workouts-exercise-detail-notes"><h3><?= e(t('workouts.notes')) ?></h3><p><?= e($sessionExerciseNotes) ?></p></section><?php endif; ?>
                            <button class="btn btn-primary workouts-exercise-detail-done" type="button" data-app-modal-close><?= e(t('workouts.done')) ?></button>
                        </div>
                    </div>
                <?php endforeach; ?>
        ...
        <?php if (!$sessionDone): ?>
            <div class="workouts-session-footer<?= $nextSessionExercise === null ? ' is-last-exercise' : '' ?>">
                <?php if ($nextSessionExercise !== null): ?>
                    <form method="post" action="/?page=workouts" class="workouts-session-continue-form" data-workout-session-action data-session-id="<?= $sid ?>">
                        <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
                        <input type="hidden" name="action" value="session_continue">
                        <input type="hidden" name="session_id" value="<?= $sid ?>">
                        <input type="hidden" name="current_session_exercise_id" value="<?= (int) ($activeSessionExercise['id'] ?? 0) ?>">
                        <input type="hidden" name="next_session_exercise_id" value="<?= (int) ($nextSessionExercise['id'] ?? 0) ?>">
                        <button type="submit" class="btn btn-primary btn-block workouts-session-continue"><span><small><?= e(t('workouts.save_and_continue')) ?></small><strong><?= e((string) ($nextSessionExercise['exercise_name'] ?? t('workouts.next_exercise'))) ?></strong></span><b aria-hidden="true"><?= activity_icon_svg('chevron-right') ?></b></button>
                    </form>
                <?php else: ?>
                    <p class="workouts-session-last-hint"><span aria-hidden="true"><?= activity_icon_svg('check') ?></span><span><strong><?= e(t('workouts.last_exercise_ready')) ?></strong><small><?= e(t('workouts.finish_from_menu_hint')) ?></small></span></p>
                <?php endif; ?>
            </div>
        <?php else: ?>
...
<?php elseif ($wkView === 'analytics' && !empty($wkStats)): ?>
    ...
    <?php if (!empty($wkStatsSession)): ?>
        ...
        $detailExercises = wk_session_completed_exercises((array) $wkStatsSessionExercises);
        $detailExerciseMediaMap = (array) ($wkStatsSessionExerciseMedia ?? []);
        ...
        $detailStartedTs = strtotime((string) ($detailSession['started_at'] ?? '')) ?: 0;
        $detailEndedTs = strtotime((string) ($detailSession['ended_at'] ?? '')) ?: $detailStartedTs;
        $detailDurationMinutes = max(0, (int) round(($detailEndedTs - $detailStartedTs) / 60));
        $detailDurationLabel = $detailDurationMinutes >= 60
            ? intdiv($detailDurationMinutes, 60) . 'h ' . ($detailDurationMinutes % 60) . 'min'
            : $detailDurationMinutes . 'min';
        $detailMuscles = [];
        foreach ($detailExercises as $dex) {
            $muscle = trim((string) ($dex['muscle_group'] ?? ''));
            if ($muscle === '') continue;
            $muscleSets = count(array_filter((array) ($dex['sets'] ?? []), static fn(array $set): bool => (int) ($set['completed'] ?? 0) === 1));
            $detailMuscles[$muscle] = ($detailMuscles[$muscle] ?? 0) + $muscleSets;
        }
        arsort($detailMuscles);
        ...
        <article class="workouts-session-detail-card">
            <header class="workouts-session-detail-author">
                <span class="workouts-session-detail-avatar"><?php if ($detailAvatarUrl !== ''): ?><img src="<?= e($detailAvatarUrl) ?>" alt="<?= e($detailOwnerName) ?>"><?php else: ?><?= e(initials_for($detailOwnerName)) ?><?php endif; ?></span>
                <div class="workouts-session-detail-author-copy"><strong><?= e($detailOwnerName) ?></strong><p><?= e(format_date_eu((string) ($detailSession['started_at'] ?? ''))) ?><?php if ($detailStartedTs > 0): ?> · <?= e(date('H:i', $detailStartedTs)) ?><?php endif; ?></p></div>
            ...
            <h2 class="workouts-session-detail-title"><?= e(wk_session_display_title($detailSession) ?: t('workouts.session')) ?></h2>
            <div class="workouts-session-detail-metrics">
                <span><small><?= e(t('feed.time')) ?></small><strong><?= e($detailDurationLabel) ?></strong></span>
        ...
        <?php if ($detailMuscles !== []): ?>
            <section class="workouts-session-muscle-split" aria-labelledby="workouts-muscle-split-title">
                <h2 id="workouts-muscle-split-title"><?= e(t('workouts.muscle_split')) ?></h2>
                <?php foreach ($detailMuscles as $muscle => $muscleSets): $musclePct = $detailTotalSets > 0 ? (int) round(($muscleSets / $detailTotalSets) * 100) : 0; ?>
                    <div class="workouts-session-muscle-row"><strong><?= e($muscleLabel($muscle)) ?></strong><div><span style="width: <?= $musclePct ?>%"></span></div><em><?= $musclePct ?>%</em></div>
                <?php endforeach; ?>
            </section>
        <?php endif; ?>
                ...
                <?php foreach ($detailExercises as $dex): ?>
                    ...
                    <article class="workouts-session-detail-exercise">
                        <div class="workouts-session-detail-exercise-head">
                            <?php $detailExerciseImage = trim((string) ($dex['image_path'] ?? '')); ?>
                            <button type="button" data-app-modal-open="workout-exercise-detail-<?= (int) ($dex['id'] ?? 0) ?>" data-workout-exercise-detail-open="workout-exercise-detail-<?= (int) ($dex['id'] ?? 0) ?>" aria-haspopup="dialog" aria-controls="workout-exercise-detail-<?= (int) ($dex['id'] ?? 0) ?>" aria-label="<?= e((string) ($dex['exercise_name'] ?? '')) ?>">
                                <?php if ($detailExerciseImage !== ''): ?><img src="<?= e(media_thumbnail_url($detailExerciseImage, 120)) ?>" alt="" loading="lazy"><?php else: ?><span class="workouts-session-detail-exercise-icon" aria-hidden="true"><?= activity_icon_svg('dumbbell') ?></span><?php endif; ?>
                                <span><strong><?= e((string) ($dex['exercise_name'] ?? '')) ?></strong><small><?= e($muscleLabel((string) ($dex['muscle_group'] ?? ''))) ?></small></span>
                            </button>
                        </div>
                        <?php if ($dexSets === []): ?>
                    ...
                    <?php
                    $detailExerciseDefId = (int) ($dex['exercise_def_id'] ?? 0);
                    $detailExerciseGallery = array_values((array) ($detailExerciseMediaMap[$detailExerciseDefId] ?? []));
                    $detailExerciseContent = (array) ($dex['content'] ?? []);
                    $detailExerciseVideo = $workoutVideoSource((string) ($dex['video_url'] ?? ''));
                    $detailSecondaryDecoded = json_decode((string) ($dex['secondary_muscles'] ?? '[]'), true);
                    $detailSecondaryMuscles = array_values(array_filter(array_map('strval', is_array($detailSecondaryDecoded) ? $detailSecondaryDecoded : [])));
                    ?>
                    <div class="app-modal workouts-exercise-detail-modal" id="workout-exercise-detail-<?= (int) ($dex['id'] ?? 0) ?>" hidden role="dialog" aria-modal="true" aria-labelledby="workout-exercise-detail-title-<?= (int) ($dex['id'] ?? 0) ?>">
                        <div class="app-modal-card workouts-exercise-detail-sheet">
                            <div class="workouts-exercise-detail-grabber" aria-hidden="true"></div>
                            <div class="workouts-exercise-detail-head"><h2 id="workout-exercise-detail-title-<?= (int) ($dex['id'] ?? 0) ?>"><?= e((string) ($dex['exercise_name'] ?? '')) ?></h2><button type="button" data-app-modal-close aria-label="<?= e(t('common.close_action')) ?>">&times;</button></div>
                            <?php if ($detailExerciseGallery !== []): ?>
                                <div class="workouts-exercise-detail-gallery">
                                    <?php foreach ($detailExerciseGallery as $galleryIndex => $galleryItem): ?><figure<?= $galleryIndex === 0 ? ' class="is-primary"' : '' ?>><img src="<?= e(media_thumbnail_url((string) ($galleryItem['path'] ?? ''), $galleryIndex === 0 ? 900 : 320)) ?>" alt="<?= e((string) ($galleryItem['caption'] ?? '')) ?>" loading="lazy"><?php if (trim((string) ($galleryItem['caption'] ?? '')) !== ''): ?><figcaption><?= e((string) $galleryItem['caption']) ?></figcaption><?php endif; ?></figure><?php endforeach; ?>
                                </div>
                            <?php elseif ($detailExerciseImage !== ''): ?><img class="workouts-exercise-detail-cover" src="<?= e(media_thumbnail_url($detailExerciseImage, 900)) ?>" alt="<?= e((string) ($dex['exercise_name'] ?? '')) ?>" loading="lazy"><?php endif; ?>
                            <dl class="workouts-exercise-detail-meta"><div><dt><?= e(t('workouts.primary_muscle')) ?></dt><dd><?= e($muscleLabel((string) ($dex['muscle_group'] ?? ''))) ?></dd></div><?php if ($detailSecondaryMuscles !== []): ?><div><dt><?= e(t('workouts.secondary_muscles')) ?></dt><dd><?= e(implode(', ', array_map($muscleLabel, $detailSecondaryMuscles))) ?></dd></div><?php endif; ?><div><dt><?= e(t('workouts.equipment')) ?></dt><dd><?= e($equipmentLabel((string) ($dex['equipment'] ?? ''))) ?></dd></div></dl>
                            <div class="workouts-exercise-detail-actions"><a class="btn btn-ghost" href="/?page=workouts&amp;view=stats&amp;exercise_stats=<?= $detailExerciseDefId ?>"><?= activity_icon_svg('chart') ?><span><?= e(t('workouts.see_your_stats')) ?></span></a><?php if ($activeRoutines !== []): ?><button class="btn btn-primary" type="button" data-app-modal-open="wk-add-exercise-modal" data-workout-routine-picker-open data-exercise-id="<?= $detailExerciseDefId ?>" data-exercise-name="<?= e((string) ($dex['exercise_name'] ?? '')) ?>"><?= activity_icon_svg('plus') ?><span><?= e(t('workouts.add_to_routine')) ?></span></button><?php endif; ?></div>
                            <?php if ($detailExerciseVideo !== null): ?><div class="workouts-exercise-detail-video"><?php if (($detailExerciseVideo['type'] ?? '') === 'iframe'): ?><iframe src="<?= e((string) ($detailExerciseVideo['url'] ?? '')) ?>" title="<?= e((string) ($dex['exercise_name'] ?? '')) ?>" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe><?php elseif (($detailExerciseVideo['type'] ?? '') === 'video'): ?><video src="<?= e((string) ($detailExerciseVideo['url'] ?? '')) ?>" controls preload="metadata"></video><?php else: ?><a class="btn btn-ghost" href="<?= e((string) ($detailExerciseVideo['url'] ?? '')) ?>" target="_blank" rel="noopener noreferrer"><?= e(t('workouts.watch_video')) ?> &nearr;</a><?php endif; ?></div><?php endif; ?>
                            <?php if ((array) ($detailExerciseContent['steps'] ?? []) !== []): ?><section class="workouts-exercise-detail-how"><h3><?= e(t('workouts.how_to')) ?></h3><ol><?php foreach ((array) $detailExerciseContent['steps'] as $step): ?><li><?= e((string) $step) ?></li><?php endforeach; ?></ol></section><?php endif; ?>
                            <button class="btn btn-primary workouts-exercise-detail-done" type="button" data-app-modal-close><?= e(t('workouts.done')) ?></button>
                        </div>
                    </div>
                <?php endforeach; ?>
...
<?php if (($wkView === 'library' && !$hasLibraryTarget || $wkView === 'analytics' && !empty($wkStatsSession)) && $activeRoutines !== []): ?>
...
<?php if ($wkView === 'library'): ?>
<div class="app-modal workouts-media-lightbox" id="workouts-media-lightbox" hidden role="dialog" aria-modal="true" aria-labelledby="workouts-media-lightbox-title">
    <div class="app-modal-card workouts-media-lightbox-card">
        <div class="app-modal-head">
            <h2 id="workouts-media-lightbox-title" data-workout-media-lightbox-title></h2>
            <button type="button" class="app-modal-close" data-app-modal-close aria-label="<?= e(t('common.close_action')) ?>">&times;</button>
        </div>
        <div class="workouts-media-lightbox-body" data-workout-media-lightbox-body></div>
    </div>
</div>
<?php endif; ?>

<?php if ($wkView === 'library' && $hasLibraryTarget): ?>
<div class="app-modal workouts-create-exercise-leave-modal" id="wk-create-exercise-leave-modal" hidden role="dialog" aria-modal="true" aria-labelledby="wk-create-exercise-leave-title">
    <div class="app-modal-card workouts-create-exercise-leave-card">
        <div class="app-modal-head">
            <h2 id="wk-create-exercise-leave-title"><?= e(t('workouts.create_custom')) ?></h2>
            <button type="button" class="app-modal-close" data-app-modal-close aria-label="<?= e(t('common.close_action')) ?>">&times;</button>
        </div>
        <span class="workouts-create-exercise-leave-icon" aria-hidden="true"><?= activity_icon_svg('plus') ?></span>
        <p><?= e(t('workouts.create_custom_leave_hint')) ?></p>
        <p class="workouts-create-exercise-leave-saved"><span aria-hidden="true"><?= activity_icon_svg('check') ?></span><span><?= e(t('workouts.create_custom_saved_hint')) ?></span></p>
        <div class="workouts-create-exercise-leave-actions"><button class="btn btn-ghost" type="button" data-app-modal-close><?= e(t('common.cancel')) ?></button><a class="btn btn-primary" href="<?= e($customExerciseUrl) ?>"><?= e(t('common.continue')) ?></a></div>
    </div>
</div>
<?php endif; ?>
...
<?php if ($hasActiveWorkoutSession): ?>
<div class="app-modal workouts-finish-session-modal" id="wk-finish-session-modal" hidden role="dialog" aria-modal="true" aria-labelledby="wk-finish-session-title">
    <div class="app-modal-card workouts-finish-session-card">
        <div class="workouts-finish-session-grabber" aria-hidden="true"></div>
        <div class="app-modal-head">
            <div><p class="eyebrow"><?= e(t('workouts.active_session')) ?></p><h2 id="wk-finish-session-title"><?= e(t('workouts.finish_options_title')) ?></h2></div>
            <button type="button" class="app-modal-close" data-app-modal-close aria-label="<?= e(t('common.close_action')) ?>">&times;</button>
        </div>
        <p class="workouts-finish-session-intro"><?= e(t('workouts.finish_options_hint')) ?></p>
        <form id="workouts-session-finish-<?= (int) $wkActiveSession['id'] ?>" method="post" action="/?page=workouts" class="workouts-finish-session-form" data-workout-session-end data-workout-session-action data-session-id="<?= (int) $wkActiveSession['id'] ?>">
            <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
            <input type="hidden" name="action" value="session_finish">
            <input type="hidden" name="session_id" value="<?= (int) $wkActiveSession['id'] ?>">
            <label class="workouts-session-count-toggle workouts-finish-count-toggle">
            ...
            <div class="workouts-finish-session-options">
                <button type="submit" name="finish_mode" value="current" class="workouts-finish-option is-primary">
                    <span class="workouts-finish-option-icon" aria-hidden="true"><?= activity_icon_svg('check') ?></span>
                    <span class="workouts-finish-option-copy"><strong><?= e(t('workouts.finish_current')) ?></strong><small><?= e(t('workouts.finish_current_hint')) ?></small></span>
                    <b aria-hidden="true">&rsaquo;</b>
                </button>
                <?php if ($activeSessionPreviousSetCount > 0): ?>
                    <button type="submit" name="finish_mode" value="previous" class="workouts-finish-option is-previous">
                        <span class="workouts-finish-option-icon" aria-hidden="true"><?= activity_icon_svg('restart') ?></span>
                        <span class="workouts-finish-option-copy"><strong><?= e(t('workouts.finish_previous')) ?></strong><small><?= e(t('workouts.finish_previous_hint', ['count' => $activeSessionPreviousSetCount])) ?></small></span>
                        <b class="workouts-finish-option-count" aria-label="<?= e(t('workouts.sets')) ?>"><?= $activeSessionPreviousSetCount ?></b>
                    </button>
                <?php endif; ?>
            </div>
        </form>
        <form method="post" action="/?page=workouts" class="workouts-finish-cancel-form" data-workout-session-end data-session-id="<?= (int) $wkActiveSession['id'] ?>" onsubmit="return confirm('<?= e(t('workouts.cancel_session')) ?>?');">
            <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
            <input type="hidden" name="action" value="session_cancel">
            <input type="hidden" name="session_id" value="<?= (int) $wkActiveSession['id'] ?>">
            <button type="submit" class="workouts-finish-option is-danger">
                <span class="workouts-finish-option-icon" aria-hidden="true"><?= activity_icon_svg('trash') ?></span>
                <span class="workouts-finish-option-copy"><strong><?= e(t('workouts.cancel_session')) ?></strong><small><?= e(t('workouts.cancel_session_hint')) ?></small></span>
                <b aria-hidden="true">&rsaquo;</b>
            </button>
        </form>
        <button type="button" class="btn btn-ghost workouts-finish-keep" data-app-modal-close><?= e(t('workouts.keep_training')) ?></button>
    </div>
</div>
<?php endif; ?>

<?php if ($wkView === 'session' && isset($sessionDone) && !$sessionDone && !empty($activeSessionExercise) && $sessionReplacementCandidates !== []): ?>
<div class="app-modal workouts-replace-exercise-modal" id="wk-replace-exercise-modal" hidden role="dialog" aria-modal="true" aria-labelledby="wk-replace-exercise-title" data-workout-replace-modal>
    <div class="app-modal-card workouts-replace-exercise-card">
        <div class="workouts-replace-exercise-grabber" aria-hidden="true"></div>
        <div class="app-modal-head">
            <div><p class="eyebrow"><?= e(t('workouts.active_session')) ?></p><h2 id="wk-replace-exercise-title"><?= e(t('workouts.replace_exercise')) ?></h2><p class="muted"><?= e(t('workouts.replace_exercise_hint')) ?></p></div>
            <button type="button" class="app-modal-close" data-app-modal-close aria-label="<?= e(t('common.close_action')) ?>">&times;</button>
        </div>
        <form method="post" action="/?page=workouts" class="workouts-replace-exercise-form" data-workout-replace-form data-allow-multi-submit>
            <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
            <input type="hidden" name="action" value="session_replace_exercise">
            <input type="hidden" name="async" value="1">
            <input type="hidden" name="session_id" value="<?= $sid ?>">
            <input type="hidden" name="session_exercise_id" value="<?= (int) ($activeSessionExercise['id'] ?? 0) ?>" data-workout-replace-session-exercise>
            <input type="hidden" name="replacement_exercise_def_id" value="" data-workout-replace-choice>
            <div class="workouts-replace-exercise-filters">
                <label class="workouts-replace-search"><span class="sr-only"><?= e(t('workouts.search_exercises')) ?></span><?= activity_icon_svg('search') ?><input type="search" placeholder="<?= e(t('workouts.search_exercises')) ?>" autocomplete="off" data-workout-replace-search></label>
                <div>
                    <label><span class="sr-only"><?= e(t('workouts.muscle_group')) ?></span><select data-workout-replace-muscle><option value=""><?= e(t('workouts.all_muscles')) ?></option><?php foreach (wk_muscle_groups() as $replaceMuscle): ?><option value="<?= e($replaceMuscle) ?>"><?= e($muscleLabel($replaceMuscle)) ?></option><?php endforeach; ?></select></label>
                    <label><span class="sr-only"><?= e(t('workouts.equipment')) ?></span><select data-workout-replace-equipment><option value=""><?= e(t('workouts.all_equipment')) ?></option><?php foreach (wk_equipment_options() as $replaceEquipment): ?><option value="<?= e($replaceEquipment) ?>"><?= e($equipmentLabel($replaceEquipment)) ?></option><?php endforeach; ?></select></label>
                </div>
            </div>
            <p class="workouts-replace-exercise-context" data-workout-replace-context><?= e(t('workouts.similar_results_hint')) ?></p>
            <div class="workouts-replace-exercise-results" data-workout-replace-results>
                <?php foreach ($sessionReplacementCandidates as $replacementExercise): ?>
                    <?php
                    $replacementExerciseId = (int) ($replacementExercise['id'] ?? 0);
                    $replacementExerciseName = (string) ($replacementExercise['display_name'] ?? $replacementExercise['name'] ?? '');
                    $replacementExerciseCover = $workoutCoverAsset((array) $replacementExercise);
                    ?>
                    <button type="button" class="workouts-replace-exercise-option" data-workout-replace-option data-exercise-id="<?= $replacementExerciseId ?>" data-name="<?= e($replacementExerciseName) ?>" data-muscle="<?= e((string) ($replacementExercise['muscle_group'] ?? '')) ?>" data-equipment="<?= e((string) ($replacementExercise['equipment'] ?? '')) ?>">
                        <span class="workouts-exercise-cover<?= $replacementExerciseCover !== null ? ' has-media' : ' is-fallback' ?>" aria-hidden="true"><?php if ($replacementExerciseCover !== null): ?><img src="<?= e((string) $replacementExerciseCover['url']) ?>" alt="" style="object-position: <?= e($workoutCoverPosition($replacementExerciseCover)) ?>" loading="lazy"><?php else: ?><?= e($workoutExerciseMark((array) $replacementExercise)) ?><?php endif; ?></span>
                        <span><strong><?= e($replacementExerciseName) ?></strong><small><?= e($muscleLabel((string) ($replacementExercise['muscle_group'] ?? ''))) ?> · <?= e($equipmentLabel((string) ($replacementExercise['equipment'] ?? ''))) ?></small></span>
                        <i aria-hidden="true"><?= activity_icon_svg('check') ?></i>
                    </button>
                <?php endforeach; ?>
            </div>
            <p class="workouts-replace-exercise-empty" data-workout-replace-empty hidden><?= e(t('workouts.no_replace_results')) ?></p>
            <p class="workouts-replace-exercise-status" data-workout-replace-status aria-live="polite"></p>
            <div class="workouts-replace-exercise-actions">
                <span data-workout-replace-selected><?= e(t('workouts.choose_replacement')) ?></span>
                <div class="workouts-replace-exercise-action-buttons">
                    <button type="button" class="btn btn-ghost" data-app-modal-close><?= e(t('common.cancel')) ?></button>
                    <button type="submit" class="btn btn-primary" data-workout-replace-submit disabled><?= e(t('workouts.replace_exercise_action')) ?></button>
                </div>
            </div>
        </form>
    </div>
...
<div class="app-modal" id="wk-new-routine-modal" hidden role="dialog" aria-modal="true" aria-labelledby="wk-new-routine-title">
    <div class="app-modal-card workouts-routine-modal-card">
        <div class="app-modal-head workouts-routine-builder-head">
            <button type="button" class="workouts-routine-modal-cancel" data-app-modal-close><?= e(t('common.cancel')) ?></button>
            <h2 id="wk-new-routine-title"><?= e(t('workouts.create_new_routine')) ?></h2>
            <button type="submit" class="workouts-routine-modal-save" form="wk-new-routine-form" data-routine-create-save disabled><?= e(t('common.save')) ?></button>
        </div>
        <form method="post" action="/?page=workouts" enctype="multipart/form-data" id="wk-new-routine-form" class="stack workouts-routine-builder-form" data-workout-media-editor data-routine-create-form>
            <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
            <input type="hidden" name="action" value="routine_create">
            <input type="hidden" name="after_create" value="" data-routine-create-after>
            <label class="workouts-routine-title-field"><span><?= e(t('workouts.routine_name')) ?></span><input type="text" name="name" maxlength="80" required placeholder="Push / Pull / Legs…"></label>

            <div class="workouts-routine-builder-empty">
                <span class="workouts-routine-builder-empty-icon" aria-hidden="true"><?= activity_icon_svg('dumbbell') ?></span>
                <p><?= e(t('workouts.routine_builder_empty')) ?></p>
                <button type="button" class="btn btn-primary btn-block" data-routine-create-add-exercise><span aria-hidden="true">+</span> <?= e(t('workouts.add_exercise')) ?></button>
            </div>

            <div class="workouts-routine-builder-options">
                <h3><?= e(t('workouts.routine_details')) ?></h3>
                ...
                <button type="submit" class="btn btn-primary btn-block" data-routine-create-submit><?= e(t('common.create')) ?></button>
            </div>
        </form>
    </div>
</div>
